                    ...
                    <form action="{{ route('popups.update',$popup->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input id="title" type="text" class="form-control @error('title') is-invalid @enderror"
                                name="title" value="{{ $popup->title }}" required
                        ...
                        <div class="form-group">
                            ...
                            <select name="show" id='visible' class="form-control">
                                <option value=0 @if($popup->show==0) selected @endif>Hidden</option>
                                <option value=1 @if($popup->show==1) selected @endif>Visible</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="message">Description</label>
                            <textarea name="message" class="ckeditor form-control" rows="5" placeholder="Event Description"
                                required> {{ $popup->message }} </textarea>
                            @error('message')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
